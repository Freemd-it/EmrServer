...
(0, _jquery2.default)('#pharmacopoeia').on('click', function () {
    ...
    _jquery2.default.ajax({
        ...
        for (var i = 0; i < result.length; i++) {
            if (result[i].primaryCategory === '심혈관계질환') {
                (0, _jquery2.default)('.main-category-select > select').nextAll('div.text').text('' + result[i].primaryCategory);
                (0, _jquery2.default)('.main-category-select > select').append('<option selected value=\'' + result[i].primaryCategory + '\'> ' + result[i].primaryCategory + ' </option>');
            } else {
