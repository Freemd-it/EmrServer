...
(0, _jquery2.default)('.pharmacySearchButton').on('click', function () {
    ...
    _jquery2.default.ajax({
        ...
        for (var i in result) {
            (0, _jquery2.default)('#medicineTableBody').append('<tr id=' + result[i].id + ' data-tooltip="Add users to your feed" data-position="left center">\n                 <td>' + result[i].name + '</td>\n                 <td>' + result[i].ingredient + '</td>\n                 <td>' + result[i].medication + '</td>\n                 <td>' + result[i].property + '</td>\n          </tr>');
        }
    });
});
